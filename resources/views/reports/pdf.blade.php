...
<div class="header">
    ...
    <div style="text-align: right; font-size: 11px; color: #7d8590;">
        ...
        @if($dateFrom || $dateTo)
        <div>Period: {{ $dateFrom ?: '—' }} to {{ $dateTo ?: 'Present' }}</div>
        @endif
    </div>
</div>
...
<div class="content">
    ...
    <div class="stats-row">
        <div class="stat-box">
            <div class="val" style="color:#1a73e8">{{ $summary['total'] }}</div>
            <div class="lbl">Total Assets</div>
        </div>
        <div class="stat-box">
            <div class="val" style="color:#2d7a3a">{{ $summary['available'] }}</div>
            <div class="lbl">Available</div>
        </div>
        <div class="stat-box">
            <div class="val" style="color:#8a6a00">{{ $summary['deployed'] }}</div>
            <div class="lbl">Deployed</div>
        </div>
        <div class="stat-box">
            <div class="val" style="color:#c13232">{{ $summary['defective'] }}</div>
            <div class="lbl">Defective</div>
        </div>
        <div class="stat-box">
            <div class="val" style="color:#6a37c0">{{ $summary['spare'] }}</div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <span class="status-chip s-{{ strtolower($asset->status) }}">{{ $asset->status }}</span>
                </td>
    ...
    <div class="footer">
        ...
        <span>Total: {{ $summary['total'] }} assets</span>
    </div>
</div>
